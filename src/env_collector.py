...
class EnvCollector:
    ...
    def collect(self, agent: Agent, epoch: int, epsilon: float, should_sample: bool, temperature: float, burn_in: int, *, num_steps: Optional[int] = None, num_episodes: Optional[int] = None):
        ...
        steps, episodes = 0, 0
        returns = []
        observations, actions, rewards, dones, states = [], [], [], [], []
        ...
        while not should_stop(steps, episodes):
            ...
            states.append(self.env.env.unwrapped.clone_state())
            ...
            if self.env.should_reset():
                self.add_experience_to_dataset(observations, actions, rewards, dones, states)
    ...
    def add_experience_to_dataset(self, observations: List[np.ndarray], actions: List[np.ndarray], rewards: List[np.ndarray], dones: List[np.ndarray], states) -> None:
        assert len(observations) == len(actions) == len(rewards) == len(dones) == len(states)
