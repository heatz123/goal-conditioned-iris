...
@dataclass
class WorldModelOutputWithActions(WorldModelOutput):
...
class WorldModel(nn.Module):
    def __init__(
        ...
        none_but_last_obs_tokens_pattern = 1 - all_but_last_obs_tokens_pattern
        ...
        self.head_actions = Head(
            max_blocks=config.max_blocks,
            block_mask=none_but_last_obs_tokens_pattern,
            head_module=nn.Sequential(
                nn.Linear(config.embed_dim, config.embed_dim),
                nn.ReLU(),
                nn.Linear(config.embed_dim, act_vocab_size + 2),
            ),
        )
    ...
    def forward(
        ...
        logits_actions = self.head_actions(
            x, num_steps=num_steps, prev_steps=prev_steps
        )

        return WorldModelOutputWithActions(
            x, logits_observations, logits_rewards, logits_ends, logits_actions
        )
    ...
    def compute_loss(
        ...
        # loss_actions_mask = (labels_acts == 2) | (labels_acts == 3)
        loss_actions = F.cross_entropy(
            rearrange(outputs.logits_actions, "b t a -> (b t) a"), labels_acts,
        )
    ...
    def compute_labels_world_model(
        ...
        labels_act = rearrange(
            act_tokens.masked_fill(mask_fill.unsqueeze(-1).expand_as(act_tokens), -100),
            "b t k -> b (t k)",
        )
        return (
